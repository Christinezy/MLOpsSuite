...
def trans(source_lang, target_lang, filename):
    ...
    print(model)
    #parser = get_parser()
    lparser = parser.parse_args(['--model_path', model,'--src_lang', source_lang, '--tgt_lang', target_lang, '--BPE_path', './TransCoder/data/BPE_with_comments_codes'])
    ...
    with torch.no_grad():
        output = translator.translate(
            input, lang1=lparser.src_lang, lang2=lparser.tgt_lang, beam_size=lparser.beam_size)
    filen.close()
    ...
    sys.stdout = open('output'+filetype, 'w')
    for out in output:
        #print("=" * 20)
        print(out)
